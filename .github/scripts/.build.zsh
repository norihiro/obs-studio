  ...
  case ${target} {
    macos-*)
      ...
      if [[ ${GITHUB_EVENT_NAME} == push && ${GITHUB_REF_NAME} =~ [0-9]+.[0-9]+.[0-9]+(-(rc|beta).+)? ]] {
        ...
        run_xcodebuild ${build_args} -target libobs
        run_xcodebuild ${build_args} -target obs-frontend-api
      ...
      devel_files=(
        ...
        $(git ls-files UI/obs-frontend-api/ | grep '\.h')
        build_macos/libobs/
        build_macos/UI/obs-frontend-api/
        build_macos/config
      )
